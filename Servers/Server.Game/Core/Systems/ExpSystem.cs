    ...
    public class ExpSystem
    {
        ...
        public void KillUnit(GameSession client, GMonster monster)
        {
            ...
            if (levelsGained > 0)
            {
                ...
                // Reset обязателен: GPc.CalcAbility, в отличие от GChar.CalcAbility (GChar.cs:154),
                // сама характеристики не обнуляет и складывает прибавки экипировки через +=, так что
                // второй вызов на той же Ability удвоил бы их. До боя её звали один раз за сессию,
                // теперь она вызывается на каждом апе
                client.Pc.Ability.Reset();
